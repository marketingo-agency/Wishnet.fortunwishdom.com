...
**REQUIRES HUMAN (Sam):** (1) **ROTATE the Supabase access token** pasted in chat (used for the v21/v22 deploys) — https://supabase.com/dashboard/account/tokens. (2) **Live paid verification** (needs your session + fal spend + eyes): a Step-6 generation with the new models honoring chosen specs (no aspect rejection), the live **credit number** on Step 5 + the agent card (if it still reads "Credits N/A" the edge log now names the unparsed billing keys — one-line follow-up to add an explicit candidate), a 1080×566 **redesign** in Step 10, and the **compare modal** (regenerate → approve replaces in place; one-image proceed).
