...
export function buildVideoInput(
  ...
  if (c?.durations) {
    ...
    input.duration = c.durationSuffix ? `${snapped}s` : c.durationAsString ? String(snapped) : snapped;
  }
  ...
  if (c?.nativeAudio && !c.audioAlwaysOn && params.generateAudio !== undefined) {
    input[c.audioFlagKey ?? 'generate_audio'] = params.generateAudio;
  }
