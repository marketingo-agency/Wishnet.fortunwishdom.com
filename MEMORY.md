...
- [PARTIAL] Phase 6 — DONE: A11Y-01, UI-05, **contrast pass UI-03/04/06** (role badges ×2, Wishpedia category, Nexus 'Not Connected', ComingSoon badge → AA), all committed + tsc/lint clean. **UI-01 = dev-only false positive** (Next.js dev-tools 'N' indicator overlaps Collapse in dev only; gone in prod — no fix). **UI-13 already handled** (ProfileHero uses initials AvatarFallback). REMAINING: UI-02 (Pixel theme), UI-07 (disabled buttons), UI-08 (mobile tab clipping), UI-09 (file tiles), UI-10 (back-nav consistency), UI-11 (stat affordance), UI-12 (agent subtitle), UI-14–19, UI-LB-01.
