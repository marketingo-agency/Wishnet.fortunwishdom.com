...
Pixel-generated images couldn't be seen/downloaded/copied. Two root causes: (1) pixel-chat stored images in the PRIVATE `files` bucket but returned `getPublicUrl()` (403s) → switched to `createSignedUrl(24h)` like osha-chat (redeployed pixel-chat); (2) `PixelOutputCard` (the studio renderer) `<img>` had `loading="lazy"` + `display:none` → never loaded (same deadlock fixed in OshaMessageBubble) → removed lazy + added onError (also fixed PixelMessageBubble). Verified live: 1024×1024 image renders, signed URL 200, Download/Copy/Save-to-Brain present.

## Durable image fix — DONE (client-only, both agents)
Resolved the 24h-expiry caveat permanently. New `useSecureImageUrl` hook + `extractFilesStoragePath` ([src/hooks/files/](src/hooks/files/)) re-sign the private `files` storage path to a fresh URL on every render (files RLS lets the owner sign — id is the first path segment). Wired into PixelOutputCard, PixelMessageBubble (img/video src, download, copy, fullscreen, Save-to-Brain) and OshaMessageBubble. Client-only — NO edge redeploy. Retroactively fixes already-stored/expired refs too. **Verified live**: with the stored image_url set to a BARE PATH, the image still rendered via a freshly client-signed URL. tsc/lint clean, committed.
