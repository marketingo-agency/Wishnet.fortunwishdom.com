...
- **Phase:** 4 — Entry fal bar + hub relayout + Surprise fold + rename
- **Step:** starting
- **Exact next action:** OmniFalHealthCard showTestButton prop + F7 contrast fix; mount in entry tiles below the grid; F6 URL guard in OmniAgent; "Inspire me" self-contained component folded into StepObjective; delete SurpriseMeView/hub card, keep useSurpriseIdeas + edge action + '?mode=surprise_me'→hub alias; rename hub card to "Studio" (own commit); add disabled Character Studio card → six-card 2×3 hub; F4 copy + F5 inert coming-soon tiles.
