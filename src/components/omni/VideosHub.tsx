...
/**
 * VideosHub: the Videos-track mode chooser (Plan 2 Phase 3 — six cards, 2×3,
 * Sam-approved structure). Mirrors OmniImagesHub's patterns; unbuilt modes
 * render visibly disabled with an honest "lands in Phase N" note (the
...
import { OMNI_VIDEO_MODES, type OmniVideoModeDef } from './omniConstants';

interface VideosHubProps {
  onBack: () => void;
  onSelectMode: (mode: OmniVideoModeDef['id']) => void;
}
...
export function VideosHub({ onBack, onSelectMode }: VideosHubProps) {
    ...
    <div className="flex h-full flex-col overflow-y-auto px-4 py-6 sm:px-8">
      ...
      >
        ...
        <h1 className="text-xl font-bold tracking-tight sm:text-2xl">
          <span className="bg-gradient-to-r from-violet-400 to-purple-500 bg-clip-text text-transparent">Videos</span>
        </h1>
          ...
          return (
            <motion.button
              ...
              className={cn(
                ...
                mode.available
                  ? 'cursor-pointer hover:-translate-y-0.5 hover:border-violet-500/40 hover:shadow-lg hover:shadow-violet-500/10'
            ...
            >
              <div className="flex items-start justify-between gap-3">
                ...
                {mode.available ? (
                  <ArrowRight className="mt-1 h-4 w-4 shrink-0 text-muted-foreground opacity-0 transition-all duration-300 group-hover:translate-x-0.5 group-hover:text-violet-400 group-hover:opacity-100" />
                ) : (
