...
export function SavePixelToBrainDialog({ open, onOpenChange, imageUrl, messageId }: SavePixelToBrainDialogProps) {
  ...
  const [destination, setDestination] = useState<'general' | string>('pixel');
  const [category, setCategory] = useState<string>('brand');
  const [saving, setSaving] = useState(false);

  useEffect(() => {
    if (open) {
      setName(`Pixel Image - ${format(new Date(), 'MMM d, yyyy HH:mm')}`);
      setDescription('');
      setDestination('pixel');
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const storagePath = `${destination === 'general' ? 'general' : destination}/${timestamp}_pixel-image.png`;
      ...
      const { data: doc, error: insertError } = await supabase
        ...
        .insert({
          ...
          name: name.trim(),
          original_name: `pixel-image-${messageId}.png`,
      ...
      runOcr({ documentId: doc.id, storagePath, mimeType });
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4 py-2">
          <div className="space-y-1.5">
            <Label htmlFor="pixel-brain-name">Name</Label>
            <Input id="pixel-brain-name" value={name} onChange={e => setName(e.target.value)} placeholder="Document name" />
          </div>

          <div className="space-y-1.5">
            <Label htmlFor="pixel-brain-desc">Description (optional)</Label>
            <Textarea id="pixel-brain-desc" value={description} onChange={e => setDescription(e.target.value)} placeholder="What does this image contain?" rows={2} className="resize-none" />
          </div>
            ...
            <Select value={destination} onValueChange={setDestination}>
              ...
              <SelectContent>
                ...
                {activeAgents.map(agent => (
                  <SelectItem key={agent.id} value={agent.id}>{agent.name} — {agent.role}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          </div>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSave} disabled={saving} className="bg-gradient-to-r from-pink-500 to-rose-600 hover:from-pink-600 hover:to-rose-700 text-white border-0">
            {saving ? (
